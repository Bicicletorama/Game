...
{
    increment *= 0.25;
    setDirection(directionAngle + increment);
}
...
{   
    ...
    float angle = bike.body->GetAngle() + (value*MAX_DIRECTION_ANGLE);
...
void player::draw() 
{
    ...
    //wheel
    spriteFront.setCurrentFrame( ofMap(directionAngle, -1, 1, 0, 8) );
...
{
    float vel = bike.getVelocity().lengthSquared();
