...
//--------------------------------------------------------------
void game::setup(int _width, int _height, b2World* _world)
{
    width = _width;
    height = _height;
    world = _world;
...
//--------------------------------------------------------------
void game::update(float guidao[TOTAL_PLAYERS], float velo[TOTAL_PLAYERS])
{   
    //contagem regressiva
    ...
    if (!locked)
    {
        //players
        for (int i=0; i<TOTAL_PLAYERS; i++) {
            playerList[i].update(guidao[i], velo[i]);
        }
    }
