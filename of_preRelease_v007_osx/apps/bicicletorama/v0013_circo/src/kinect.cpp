...
void kinect::setup(int _width, int _height, b2World* _world)
{
	...
	fullSize = new unsigned char[kinectWidth*kinectHeight];
	fullSize2 = new unsigned char[kinectWidth*kinectHeight];
	transImage.allocate(kinectWidth, kinectHeight);
	finalImage.allocate(kinectWidth, kinectHeight);
...
void kinect::update()
{
	...
	memset(fullSize, 0, kinectWidth*kinectHeight);	
	memset(fullSize2, 0, kinectWidth*kinectHeight);
	...
	if( kinect.isFrameNew() ) 
	{
		...
		unsigned char * pixels = kinectOutputCropped.getPixels();
		
		for (int y=0; y < kinectHeight; y++)
		{
			for(int x=0; x< kinectWidth; x++)
			{
				int r = pixels[ y*kinectWidth + x ];
				
				if ( r == 255 )
				{
					fullSize[ y*kinectWidth + x ] = 255;
				}
			}
		}
		...
		for (int y=0; y < kinectHeight; y++)
		{
			for(int x=0; x< kinectWidth; x++)
			{
				int r = fullSize[ y*kinectWidth + x ];
				
				if ( r == 255 )
				{
					if ( y > sizer && y < (kinectHeight-sizer) && x > sizer && x < (kinectWidth-sizer) )
					{
						for ( int yy = 1; yy <= sizer; yy ++ )
						{
							fullSize2[ (y-yy)*kinectWidth + (x-yy) ]  = 255;
							fullSize2[ (y+yy)*kinectWidth + (x-yy) ]  = 255;
		...
		transImage.setFromPixels(fullSize,kinectWidth, kinectHeight);
		finalImage.setFromPixels(fullSize2,kinectWidth, kinectHeight);
		
		scaledImage.scaleIntoMe(finalImage);
		
		contourFinder.findContours( scaledImage, minBlobSize, 2000000.0, 20, false );
...
void kinect::draw()
{
	...
	//transImage.draw( 20, 20, 320, 240 );
	//finalImage.draw( 360, 20, 320, 240 );
}
